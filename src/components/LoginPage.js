...
export default function LoginPage() {
    ...
    const getData = async () => {
        await axios
        .get("/api/getAllUsers")
        .then((response) => setUsers(response.data)).then(console.log(users))
            ...
            <div className="w-full mb-2 border border-gray-300 border-solid rounded shadow-sm">
                ...
                <div className="p-3">
                    ...
                    <div className="w-full mt-10 mb-2">
                        <button onClick={Login} className="float-right px-4 py-2 font-bold text-white bg-green-500 rounded-full hover:bg-green-700">
                        Login
                        </button>
                    </div>
                </div>
